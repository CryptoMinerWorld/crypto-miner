...
library Random {
   ...
   * @return random value in range [offset, offset + length)
   */
  function __randomValue(uint256 seed, uint64 offset, uint64 length) internal view returns (uint256) {
   ...
   * @return random value in range [offset, offset + length)
   */
  function __quadraticRandom(uint256 seed, uint64 offset, uint64 length) internal view returns (uint256) {
    // using `__rawRandom` as a source of randomness,
    uint256 randomness = __rawRandom(seed);
   ...
   */
  function __rawRandom(uint256 seed) internal view returns (uint256) {
