...
contract('GeodeSale: Gas Usage', function(accounts) {
	it("Gem ERC721: deploying a GemERC721 requires 5054808 gas", async function() {
		const token = await Token.new();
		const txHash = token.transactionHash;
		const txReceipt = await web3.eth.getTransactionReceipt(txHash);
		const gasUsed = txReceipt.gasUsed;

		assertEqual(5054808, gasUsed, "deploying a GemERC721 gas usage mismatch: " + gasUsed);
	});

	it("geode sale: buying a geode requires 638327 gas", async function() {
		const token = await Token.new();
		...
		assertEqual(638327, gasUsed, "buying a geode gas usage mismatch: " + gasUsed);
	});

	it("geode sale: buying 10 geodes requires 6248231 gas", async function() {
		const token = await Token.new();
		const sale = await Sale.new(token.address, accounts[9]);
		...
		assertEqual(6248231, gasUsed, "buying a geode gas usage mismatch: " + gasUsed);
	});
});
